...
class RoomTeamServices:
    ...
    async def create_room(
        self, user_id: str, data: CreateRoomIn
    ) -> str | HTTPException:
    ...
    async def add_people_to_room(
        ...
        if result := await self.repository.add_people_to_room(
            ...
            return HTTPException(
                ...
                detail="Пользователи не добавились",
            )
    ...
    async def create_team(self, data):
        return await self.repository.create_team(
            room_id=data.room_id, name=data.name, data=data.list_users
        )
