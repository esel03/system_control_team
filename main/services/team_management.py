...
from main.schemas.team_management import CreateRoomIn, AddToRoomIn, CreateTeamIn, AddToTeamIn, DeleteTeamPeople, DeleteRoomPeople, CreateRoomOut, UserListRoom, UsersList, NumUsId, AddToTeamOut
...
class RoomTeamServices:
    ...
    async def access_right_user_in_room(self, user_id: UUID, room_id: UUID) -> bool:
        ...
        if not (await self.repository.get_info_about_user_in_room(user_id=user_id, room_id=room_id)):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Нет достаточных прав на комнату"
            )
        return True
    ...
    async def access_right_user_in_team(self, user_id: UUID, team_id: UUID) -> bool:
        ...
        Если is_chief == True -> вернется True
        '''
        if not (await self.repository.get_info_about_user_in_team(user_id=user_id, team_id=team_id)):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Нет достаточных прав на команду"
            )
        return True
    ...
    async def create_room(self, user_id: UUID, name: str) -> UUID:
        ...
        if result := await self.repository.create_room(name=name):
            ...
            if await self.repository.write_users_to_room_safe(
                data=values, room_id=result) == True:
                return CreateRoomOut(room_id=result)
            else:
                raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Комната не создалась, попробуйте еще раз"
            )
        else:
            raise HTTPException(
    ...
    async def add_people_to_room(self, room_id: UUID, data: list[UserListRoom]) -> UUID | None:
        '''
    ...
    async def create_team(self, user_id: UUID, room_id: UUID, name: str):        
        ...
        return await self.repository._write_users_to_teams(
            team_id=result, room_id=room_id, name=name, data=[UsersList(user_id=user_id, is_chief=True)])
    ...
    async def add_people_to_team(self, user_id: UUID, data: AddToTeamIn) -> UUID:
        ...
        if amount.existing_user_id >= amount.in_data:
            ...
            result_list_no_room =[uid for uid in (amount.in_data - amount.existing_user_id)] # список юзеров, которых нет в комнате

            # проверка на права юзера на комнату
            if await self.repository.get_info_about_user_in_room(user_id=user_id, room_id=room_id) == True:
                await self.add_people_to_room(room_id=room_id, data=[UserListRoom(user_id=uid, is_chief=False) for uid in result_list_no_room])
                await self.repository._write_users_to_teams(
                team_id=data.team_id, 
                room_id=room_id, 
                name=data.name, 
                ...
                result_list =[uid for uid in amount.existing_user_id] # список юзеров, которые есть в комнате
                if not result_list:
                    raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="Отсутствуют права на добавление некоторых пользователей в комнату, и соответственно - в команду"
                    )
                input_value = []
