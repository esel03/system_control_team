...
from main.schemas.team_management import UsersList
...
class RoomTeamRepository:
    ...
    async def create_room(self, data: dict, user_id: str) -> str | None:
        stmt = Room(name=data.name, list_role=data.list_role, list_tag=data.list_tag)
        self.db.add(stmt)
    ...
    async def _write_users_to_rooms(self, user_id: str, room_id: str) -> None:
        stmt = insert(UsersToRooms).values([{"user_id": user_id, "room_id": room_id}])
